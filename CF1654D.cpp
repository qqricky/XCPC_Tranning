...
void dfss(int now,int fa,ll up,ll down){
    for(int j=0;j<mpp[now].size();j++){
        int u=mpp[now][j];
        auto p=mp[now][j];
        if(u==fa)continue;
        ll uup=(up*p.second)%mod;
        ll ddown=(down*p.first)%mod;
        //cout<<uup<<'/'<<ddown<<'\n';
        ans=(ans+x*uup%mod*powq(ddown,mod-2)%mod)%mod;
        dfss(u,now,uup,ddown);
    }
}
int main()
{
