...
bool cmppp(Point a,Point b,Point p){
    double xx=p.x;
    double yy=p.y;
...
bool cmpp(node a,node b,Point p){
    return cmppp(a.x,b.x,p);
}
vector<node>v;
int main()
{
    ...
    for(int i=1;i<=n;i++){
        ...
        pp.x+=c[i].r;
        for(int j=1;j<i;j++){
            // int tp=c[i].relationcircle(c[j]);
            // if(tp==2&&)
            int num=c[i].pointcrosscircle(c[j],p1,p2);
            if(num==2){
                v.push_back();
                v.push_back();
            }
        ...
        //     }
        //     if(now){
        //         double ra=c[i].p.rad(v[j].x,v[j-1].x);
        //         res+=ra*c[i].r;
        //     }
        // }
        // if(s.size()){
        //     double ra=c[i].p.rad(v[0].x,v[v.size()-1].x);
        //     res+=ra*
        // }
        ans+=res;
    }
