...
int main()
{
    ...
    while(t--){
        ...
        double alpha=acos(2.0/dis);
        Point p1=c;
        Point p2=c;
        p1=p1.rotate(b,-alpha);
        p2=p2.rotate(b,alpha);
        Line l1=Line(a,p1);
        Line l2=Line(a,p2);
        double dis2=b.distance(c);
        double bt=asin(2.0/dis2);
        bt/=2.0;
        Point p3=c;
        Point p4=c;
        p3=p3.rotate(b,bt);
        ...
        // cout<<"\najsdkf;jkal;sj"<<'\n';
        cout<<endl;
        double x1=cross(b,p1,p3)*cross(b,p2,p3);
        double x2=cross(b,p1,p4)*cross(b,p2,p4);
        cout<<x1<<' '<<x2<<'\n';
        if(x1<0||x2<0)flag=1;
        if(flag)cout<<"yes\n";
        else cout<<"no\n";
